...
public interface CustomerRepository extends CrudRepository<Customer, Long> {
	...
	@Query(value = " select ce.id,  oi.orderguid, (ce.SEATS - ce.NUMBERSEATSUSED) as remainingSeats , u.firstname, u.lastname, u.emailaddress, sk.name as subscriptionName, oi.OrderDate " +
			" FROM CUSTOMERENTITLEMENT ce   " +
	...
	@Query(value = " SELECT  ce.id,  oi.orderguid, (ce.SEATS - ce.NUMBERSEATSUSED) as remainingSeats, sk.name,  oi.OrderDate   " +
			" FROM CUSTOMERENTITLEMENT ce   " +
			" inner join OrderLineItem oli on oli.ENTITLEMENT_ID = ce.id   " +
			" inner join orderinfo oi on oi.id = oli.ORDER_ID and ce.CUSTOMER_ID = oi.Customer_ID " +
			" Inner join subscription s on ce.id=s.CUSTOMERENTITLEMENT_id  " +
	...
	@Query(value = " SELECT ce.id as cusId, u.firstname, u.lastname, u.emailaddress, c.name as courseName, oi.OrderDate, oi.orderguid  " +
			" FROM vu360user u   " +
			" inner join Learner l on l.vu360user_id=u.id   " +
	...
	@Query(value = " SELECT ce.id,  oi.orderguid, (ce.SEATS - ce.NUMBERSEATSUSED) as remainingSeats, c.name, oi.OrderDate   " +
			" FROM CUSTOMERENTITLEMENT ce    " +
