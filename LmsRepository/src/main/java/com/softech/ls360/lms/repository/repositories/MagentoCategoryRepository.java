...
public interface MagentoCategoryRepository extends CrudRepository<MagentoCategory, Long>  {
	@Query(value=" select  id, guid, name from Course where guid in ( select COURSEGUID " +
	...
	@Query(value=" select CARRIERPATH_ID, CARRIERPATH_NAME, DESCRIPTION, CARRIERPATH_URL, CATEGORY_ID, STUFF( " +
			 " (SELECT ',' + COURSE_GUID "+
			 " FROM MAGENTO_CARRIERPATH_COURSE cc "+
			 " WHERE c.CARRIERPATH_ID = cc.CARRIERPATH_ID "+
			 " FOR XML PATH('') "+
